...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your My First API account.<br>
        ...
        &mdash; My First API Team
      `
      return sendMail({ toEmail: email, subject: 'My First API - Password Reset', content })
    })
